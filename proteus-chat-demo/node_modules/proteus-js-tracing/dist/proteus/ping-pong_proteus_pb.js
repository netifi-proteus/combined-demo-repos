...
var PingPongServiceClient = function () {
  ...
  PingPongServiceClient.prototype.ping = function ping(message, metadata) {
    ...
    var map = {};
    return this.pingTrace(map)(new rsocket_flowable.Single(function (subscriber) {
  ...
  PingPongServiceClient.prototype.pingStream = function pingStream(message, metadata) {
    ...
    var map = {};
    return this.pingStreamTrace(map)(new rsocket_flowable.Flowable(function (subscriber) {
      var dataBuf = Buffer.from(message.serializeBinary());
      var tracingMetadata = proteus_tracing.mapToBuffer(map);
...
var PingPongServiceServer = function () {
  ...
  PingPongServiceServer.prototype.requestResponse = function requestResponse(payload) {
    try {
      ...
      switch (method) {
        case 'ping':
          return this.pingTrace(spanContext)(this._service.ping(proteus_ping$pong_pb.Ping.deserializeBinary(payload.data), payload.metadata).map(function (message) {
            return {
  ...
  PingPongServiceServer.prototype.requestStream = function requestStream(payload) {
    try {
      ...
      switch (method) {
        case 'pingStream':
          return this.pingStreamTrace(spanContext)(this._service.pingStream(proteus_ping$pong_pb.Ping.deserializeBinary(payload.data), payload.metadata).map(function (message) {
            return {
              data: Buffer.from(message.serializeBinary()),
              metadata: Buffer.alloc(0)
            };
