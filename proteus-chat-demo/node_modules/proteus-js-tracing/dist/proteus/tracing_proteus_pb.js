...
var ProteusTracingServiceClient = function () {
  ...
  ProteusTracingServiceClient.prototype.streamSpans = function streamSpans(messages, metadata) {
    ...
    var once = false;
    var map = {};
    return this.streamSpansTrace(map)(new rsocket_flowable.Flowable(function (subscriber) {
  ...
  ProteusTracingServiceClient.prototype.sendSpan = function sendSpan(message, metadata) {
    ...
    var map = {};
    return this.sendSpanTrace(map)(new rsocket_flowable.Single(function (subscriber) {
      var dataBuf = Buffer.from(message.serializeBinary());
      var tracingMetadata = proteus_tracing.mapToBuffer(map);
...
var ProteusTracingServiceServer = function () {
  ...
  ProteusTracingServiceServer.prototype.requestResponse = function requestResponse(payload) {
    try {
      ...
      switch (method) {
        case 'SendSpan':
          return this.sendSpanTrace(spanContext)(this._service.sendSpan(zipkin_proto3_zipkin_pb.Span.deserializeBinary(payload.data), payload.metadata).map(function (message) {
            return {
